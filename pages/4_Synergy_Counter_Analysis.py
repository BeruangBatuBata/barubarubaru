...
analysis_mode = st.sidebar.radio("Select Analysis Mode:", ["Synergy (Best Pairs)", "Anti-Synergy (Worst Pairs)", "Counters"])
...
if analysis_mode in ["Synergy (Best Pairs)", "Anti-Synergy (Worst Pairs)"]:
    ...
    if team_filter != "All Teams":
        focus_on_team_picks = st.sidebar.radio(f"Focus on matchups where:", [f"{team_filter} picks the 'Ally Hero'", f"{team_filter} plays against the 'Ally Hero'"]) == f"{team_filter} picks the 'Ally Hero'"
