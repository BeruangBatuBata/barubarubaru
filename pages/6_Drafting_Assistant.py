...
### --- ADDED: HISTORICAL ANALYSIS FEATURE --- ###
with st.expander("Review a Past Game"):
    # Create a list of playable games for the dropdown
    playable_games = []
    ...
    selected_game = st.selectbox("Select a past game to analyze:", [None] + playable_games, format_func=lambda x: x[0] if x else "None")
    ...
    if selected_game:
        ...
        # Use st.rerun() to force the UI to update with the loaded draft
        st.rerun()
...
with blue_col:
    ...
    for i in range(5):
        available = [None] + [h for h in ALL_HEROES if h not in taken_heroes or h == draft['blue_bans'][i]]
    ...
    st.subheader("Picks")
    for role in ROLES:
        available = [None] + [h for h in ALL_HEROES if h not in taken_heroes or h == draft['blue_picks'][role]]
...
with red_col:
    ...
    ban_cols = st.columns(5)
    for i in range(5):
        available = [None] + [h for h in ALL_HEROES if h not in taken_heroes or h == draft['red_bans'][i]]
    ...
    st.subheader("Picks")
    for role in ROLES:
        available = [None] + [h for h in ALL_HEROES if h not in taken_heroes or h == draft['red_picks'][role]]
...
with prob_placeholder.container():
    ...
    st.progress(prob_overall)
    st.markdown("**Draft-Only Prediction (Team Neutral)**")
    st.progress(prob_draft_only)
...
total_bans = len(blue_b) + len(red_b)
total_picks = len(blue_p) + len(red_p)
